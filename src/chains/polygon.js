/**
 * src/chains/polygon.js
 *
 * Polygon Mainnet chain adapter.
 *
 * Exports:
 *   name          — 'polygon'
...
export const viemChain = polygon;
export const nativeSymbol = 'POL';
...
async function resolveTokenList() {
  ...
  _tokenList = await getTokenList('polygon');
  return _tokenList;
}
...
/**
 * Fetch native POL (ex-MATIC) balance + top ERC-20 balances for an address.
 *
...
export async function getBalances({ address, rpcQuorum = 'any' } = {}) {
  try {
    ...
    const response = {
      ...buildBalanceResponse(name, 'POL', formatUnits(nativeWei, 18), 18, tokens),
      source,
